...
3. Use `world-map.json` for the current atlas. Preserve stable IDs. `regional-map.json` is the historical measured survey: its original sites, routes and distances remain controlling. New settlement markers allocate residents from their group’s rural/unmapped return. Explicitly authorised coverage corrections, such as revision52’s omitted outer chains, require a separate audited baseline correction; never count those residents again.
